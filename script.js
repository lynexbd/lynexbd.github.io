// ======================================================
// LYNEX MAIN SCRIPT (Max 3 Images & Slider Fixed)
// ======================================================
...
async function loadProductsDisplay(isHome) {
    ...
    grid.innerHTML = p.length ? p.map(i => {
        ...
        let arrowsHTML = '';
        if(images.length > 1) {
            arrowsHTML = `<a class="prev" onclick="changeSlide(-1, '${i.id}')">&#10094;</a><a class="next" onclick="changeSlide(1, '${i.id}')">&#10095;</a>`;
        }

        window.slideIndex[i.id] = 0; // Set initial slide index
...
// --- ADMIN: MULTIPLE IMAGE UPLOAD (Limit 3) ---
function initAdminProducts() {
    ...
    window.previewImages = () => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            reader.onload = (e) => {
                preview.innerHTML += `<img src="${e.target.result}" style="width:50px;height:50px;object-fit:cover;border-radius:4px;border:1px solid #ff9f43;">`;
            };
...
// ... (Dashboard, Orders, Messages remain same) ...
