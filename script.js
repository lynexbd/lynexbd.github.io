// ======================================================
// LYNEX MAIN SCRIPT (IndexedDB - Unlimited Storage)
// ======================================================

// --- DATABASE CONFIG ---
const DB_NAME = "LynexDB_Unlimited";
const DB_VERSION = 1;
...
// --- INDEXEDDB HELPER FUNCTIONS (ASYNC) ---
let db;
...
function setStorage(key, data) {
    return new Promise((resolve) => {
        ...
        req.onerror = (e) => {
            alert("Error saving data: " + e.target.error);
            resolve(false);
        };
    });
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (loginForm) {
        ...
        loginForm.addEventListener('submit', (e) => {
            ...
            } else { alert('Access Denied!'); e.target.reset(); }
        });
    }
});
...
async function updateAdminSidebarBadges() {
    ...
    if(orders.some(o => o.status === 'Pending') && document.getElementById('nav-orders') && !location.pathname.includes(PAGE_ORDERS)) 
        document.getElementById('nav-orders').innerHTML += ' <span class="nav-badge"></span>';
    
    if(msgs.some(m => !m.isRead) && document.getElementById('nav-messages') && !location.pathname.includes(PAGE_MESSAGES)) 
...
async function loadProductsDisplay(isHome) {
    ...
    products.forEach(p => {
        ...
        // Check if Old Price exists and is greater than current price
        ...
        // [FIXED] Buttons Text: 'Add to Cart' & 'Buy Now'
...
async function loadCartDisplay() {
    ...
    if(cart.length===0) { 
        c.innerHTML='<p style="text-align:center;color:#aaa;">Your cart is empty.</p>'; 
        if(t) t.innerText='0'; 
...
// [FIXED] Checkout Form & Detailed Confirmation
function handleCheckoutForm() {
    ...
    if (form) {
        form.onsubmit = async (e) => {
            ...
            // Order ID
            let count = await getStorage(KEY_ORDER_COUNT);
            // If KEY_ORDER_COUNT returns array (first time), reset to 0
            ...
            const deliveryMsg = "\n* ডেলিভারি চার্জ এবং বিস্তারিত তথ্যের জন্য শীঘ্রই আপনাকে কল করা হবে।";
...
function handleContactForm() {
    ...
    if(f) {
        f.onsubmit = async (e) => {
            ...
            const ms = await getStorage(KEY_MESSAGES); ms.unshift(m); await setStorage(KEY_MESSAGES, ms); e.target.reset(); alert('Sent!');
        };
    }
}
...
function initAdminProducts() {
    ...
    const render = async () => {
        ...
        tb.innerHTML = p.map((x, i) => `<tr><td><img src="${x.image||''}" style="width:40px;height:40px;object-fit:cover;border-radius:4px;background:#333;"></td><td>${x.name} ${x.isNewArrival?'<span style="color:#2ecc71;">(New)</span>':''}</td><td>৳ ${x.price}</td><td><button onclick="delP(${i})" style="color:red;border:none;background:none;cursor:pointer;">Del</button></td></tr>`).join('');
    ...
    if(f) {
        f.addEventListener('submit', (e) => {
            ...
            const save = async (imgData) => {
                ...
                await setStorage(KEY_PRODUCTS, p); e.target.reset(); render(); alert('Product Added!');
            };
...
function initAdminMessages() {
    ...
    const render = async () => {
        ...
        if (l.length === 0) { tb.innerHTML = '<tr><td colspan="5" style="text-align:center;">No Messages</td></tr>'; }
            ...
            tb.innerHTML = l.map(m => {
                ...
                return `<tr><td>${m.date}</td><td>${m.name}<br><small style="color:#aaa;">${m.email}</small></td><td>${m.subject}</td><td>${m.text}</td><td>${!m.isRead?`<button onclick="mkR(${idx})" style="color:green;background:none;border:none;cursor:pointer;">Mark Read</button>`:''}<button onclick="delMsg(${idx})" style="color:red;background:none;border:none;cursor:pointer;">Del</button></td></tr>`;
            }).join('');
        }
    };
    render();
    ...
    window.delMsg = async (i) => { if(confirm('Delete?')) { const m = await getStorage(KEY_MESSAGES); m.splice(i, 1); await setStorage(KEY_MESSAGES, m); render(); }};
}
