// ======================================================
// LYNEX MAIN SCRIPT (Final Complete & Fixed Version)
// ======================================================
...
// --- LOGIN CREDENTIALS (SysMaster_99 / L7n@x#Super!2025) ---
const _u = "SysMaster_99";
const _p = "L7n@x#Super!2025";

// --- INDEXEDDB SETUP (UNLIMITED STORAGE) ---
const DB_NAME = "LynexDB_Ultimate";
const DB_VERSION = 1;
let db;
...
async function setStorage(key, data) {
    return new Promise((resolve) => {
        ...
        req.onerror = (e) => { alert("Data Save Error: " + e.target.error); resolve(false); };
    });
}

// --- APP INITIALIZATION ---
document.addEventListener('DOMContentLoaded', async function() {
    await initDB(); // Connect DB
    createPopupHTML(); // Init Popup
    ...
    if (page === 'index.html' || page === '') await loadProductsDisplay(true); // Home
    else if (page === 'products.html') await loadProductsDisplay(false); // All Products
...
function createPopupHTML() {
    if(!document.querySelector('.custom-popup-overlay')) {
        const p = document.createElement('div'); p.className = 'custom-popup-overlay';
...
// ==========================================
//  WEBSITE LOGIC (Slider, Cart, Checkout)
// ==========================================
...
async function loadProductsDisplay(isHome) {
    ...
    grid.innerHTML = p.length ? p.map(i => {
        ...
        <div class="product-card">
            ${badgeHTML}
            <div class="slider-container" id="slider-${i.id}">
                ${slides}${arrows}${dots}
            </div>
...
async function loadCartDisplay() {
    ...
    c.innerHTML = cart.map((item, i) => {
        let thumb = Array.isArray(item.images) && item.images.length ? item.images[0] : (item.image || '');
        return `
        <div class="cart-item">
            <div class="cart-item-info">
                <img src="${thumb}" style="width:60px;height:60px;object-fit:cover;border-radius:4px;background:#333;">
                <div><h4>${item.name}</h4><p>৳${item.price} x ${item.qty}</p>
                <div class="qty-controls"><button class="qty-btn" onclick="upQty(${i},-1)">-</button><span>${item.qty}</span><button class="qty-btn" onclick="upQty(${i},1)">+</button></div></div>
            </div>
            <div style="text-align:right;"><p style="font-weight:bold;color:#ff9f43;">৳${item.price*item.qty}</p><button onclick="rmC(${i})" style="color:#e74c3c;background:none;border:none;cursor:pointer;margin-top:5px;">Remove</button></div>
        </div>`;
    }).join('');
...
function handleContactForm() {
    ...
    if(f) {
        f.onsubmit = async (e) => {
            ...
            const ms = await getStorage(KEY_MESSAGES); ms.unshift(m); await setStorage(KEY_MESSAGES, ms); e.target.reset(); showPopup('Sent', 'Message Sent!', 'success');
        };
    }
}
...
function initAdminProducts() {
    ...
    window.previewImages = () => {
        ...
        files.forEach(file => {
            ...
            r.onload = (e) => { preview.innerHTML += `<img src="${e.target.result}" style="width:50px;height:50px;object-fit:cover;border:1px solid #ff9f43;">`; };
    ...
    if(f) {
        f.addEventListener('submit', async (e) => {
            ...
            const p = await getStorage(KEY_PRODUCTS);
            p.push({ id: Date.now(), name: e.target.name.value, price: parseFloat(e.target.price.value), originalPrice: e.target.oldPrice.value ? parseFloat(e.target.oldPrice.value) : null, isNewArrival: e.target.isNew.checked, images: imgData });
            await setStorage(KEY_PRODUCTS, p); e.target.reset(); preview.innerHTML = ''; render(); showPopup('Success', 'Product Added!', 'success');
        });
    }
...
function initAdminMessages() {
    ...
    window.delMsg=async(i)=>{ if(confirm('Delete?')){ const m=await getStorage(KEY_MESSAGES); m.splice(i,1); await setStorage(KEY_MESSAGES, m); ren(); }};
}
