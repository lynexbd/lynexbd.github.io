// ======================================================
// LYNEX SCRIPT (Multi-Image Slider & Dot Navigation)
// ======================================================
...
async function updateCartCount() {
    ...
    const t = c.reduce((sum, i) => sum + (parseInt(i.qty)||0), 0);
...
// --- SLIDER LOGIC (GLOBAL) ---
window.slideIndex = {}; // Store current slide index for each product

window.changeSlide = (n, prodId) => {
    showSlides(window.slideIndex[prodId] += n, prodId);
}

window.currentSlide = (n, prodId) => {
    showSlides(window.slideIndex[prodId] = n, prodId);
}

function showSlides(n, prodId) {
    let i;
    let container = document.getElementById("slider-" + prodId);
    let slides = container.getElementsByClassName("slider-image");
    let dots = container.getElementsByClassName("dot");
    
    if (n > slides.length) {window.slideIndex[prodId] = 1}    
    if (n < 1) {window.slideIndex[prodId] = slides.length}
    ...
    slides[window.slideIndex[prodId]-1].classList.add("active");  
    dots[window.slideIndex[prodId]-1].classList.add("active");
}


// --- WEBSITE DISPLAY (SLIDER ADDED) ---
async function loadProductsDisplay(isHome) {
    ...
    grid.innerHTML = p.length ? p.map(i => {
        ...
        // Image Logic (Single vs Multiple)
        // Support old data (string) and new data (array)
        ...
        let slidesHTML = images.map((img, idx) => 
            `<img src="${img}" class="slider-image ${idx===0?'active':''}" alt="${i.name}">`
        ).join('');

        // Generate Dots
        let dotsHTML = images.map((_, idx) => 
            `<span class="dot ${idx===0?'active':''}" onclick="currentSlide(${idx+1}, '${i.id}')"></span>`
        ).join('');

        // Init Slider State
        window.slideIndex[i.id] = 1;

        // Hide controls if single image
        const sliderClass = images.length > 1 ? '' : 'single-slide';

        return `
        <div class="product-card">
            ${badgeHTML}
            
            <div class="slider-container ${sliderClass}" id="slider-${i.id}">
                ${slidesHTML}
                
                <a class="prev" onclick="changeSlide(-1, '${i.id}')">&#10094;</a>
                <a class="next" onclick="changeSlide(1, '${i.id}')">&#10095;</a>
...
function initAdminProducts() {
    ...
    const ren=async()=>{ const p=await getStorage(KEY_PRODUCTS); tb.innerHTML=p.length ? p.map((x,i)=> {
        // Show first image as thumb
        let thumb = Array.isArray(x.images) && x.images.length > 0 ? x.images[0] : (x.image || '');
        return `<tr><td><img src="${thumb}" style="width:40px;height:40px;object-fit:cover;border-radius:4px;"></td><td>${x.name} ${x.isNewArrival?'<span style="color:#2ecc71;">(New)</span>':''}</td><td>৳${x.price}</td><td><button onclick="delP(${i})" style="color:red;border:none;background:none;cursor:pointer;">Del</button></td></tr>`;
    }).join('') : '<tr><td colspan="4" style="text-align:center;">Empty</td></tr>'; document.getElementById('current-product-count').innerText=p.length; };
    ren();
    if(f) {
        f.addEventListener('submit', async (e) => {
            ...
            e.target.reset();
            ren();
            showPopup('Success', 'Product Added with Images!', 'success');
        });
    }
