// ======================================================
// LYNEX MAIN SCRIPT (Unlimited Upload Size Version)
// ======================================================
...
function setStorage(key, data) {
    try {
        ...
        // স্টোরেজ ফুল হলে বা ফাইল অনেক বড় হলে এই এরর দেখাবে
        if (e.name === 'QuotaExceededError') {
            alert('Storage Full! ফাইলটির সাইজ অনেক বড়। দয়া করে ছোট সাইজের ছবি ব্যবহার করুন অথবা পুরনো ডাটা ডিলিট করুন।');
        } else {
...
window.addToCart = (id) => {
    ...
    if (p) { 
        ...
        if(setStorage(KEY_CART, c)) { updateCartCount(); alert('Added!'); }
    }
};
...
// PRODUCT UPLOAD (NO LIMIT)
function initAdminProducts() {
    const form = document.getElementById('add-product-form');
    const tbody = document.querySelector('#product-table tbody');
    const render = () => {
        const p = getStorage(KEY_PRODUCTS);
        tbody.innerHTML = p.length ? p.map((x, i) => `<tr><td><img src="${x.image||''}" style="width:40px;height:40px;object-fit:cover;background:#333;border-radius:4px;"></td><td>${x.name} ${x.isNewArrival?'<span style="color:#2ecc71;">(New)</span>':''}</td><td>৳ ${x.price}</td><td><button onclick="delProd(${i})" style="color:red;border:none;background:none;cursor:pointer;">Del</button></td></tr>`).join('') : '<tr><td colspan="4" style="text-align:center;">Empty</td></tr>';
    ...
    if(form) {
        form.addEventListener('submit', (e) => {
            ...
            const save = (imgData) => {
                ...
                // এখানে আর ফাইল সাইজ চেক করা হচ্ছে না
                if(setStorage(KEY_PRODUCTS, p)) {
                    e.target.reset(); render(); alert('Product Added Successfully!');
                }
            };
    ...
    window.delProd = (i) => { if(confirm('Delete?')) { const p=getStorage(KEY_PRODUCTS); p.splice(i,1); setStorage(KEY_PRODUCTS, p); render(); } };
}
...
function initAdminOrders() {
    ...
    const render = () => {
        ...
        const list = currentFilter === 'All' ? all : all.filter(o => o.status === currentFilter);
        
        // Tab Styles
        document.querySelectorAll('.filter-btn').forEach(btn => {
            if(btn.innerText.includes(currentFilter) || (currentFilter === 'All' && btn.innerText === 'All')) btn.classList.add('active');
