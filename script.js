// ======================================================
// LYNEX MAIN SCRIPT (Dots Below Image Fixed)
// ======================================================
...
// --- DATABASE ---
const DB_NAME = "LynexDB_Ultimate_Final";
const DB_VERSION = 1;
let db;
...
async function loadProductsDisplay(isHome) {
    ...
    grid.innerHTML = p.length ? p.map(i => {
        ...
        // Dots are now OUTSIDE the slider container for positioning below
        let dotsHTML = images.length > 1 ? `<div class="slider-dots" id="dots-${i.id}">${images.map((_, idx) => `<span class="dot ${idx===0?'active':''}" onclick="scrollToSlide(${idx}, '${i.id}')"></span>`).join('')}</div>` : '';
