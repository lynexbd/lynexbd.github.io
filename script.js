// ======================================================
// LYNEX MAIN SCRIPT (Feedback Message Fixed)
// ======================================================
...
// --- DATABASE ---
const DB_NAME = "LynexDB_Ultimate_Final_V3"; // Version Updated
const DB_VERSION = 1;
let db;
...
async function loadProductsDisplay(isHome) {
    ...
    grid.innerHTML = p.length ? p.map(i => {
        ...
        return `<div class="product-card">${badgeHTML}<div class="image-wrapper"><div class="slider-container" id="slider-${i.id}" onscroll="updateActiveDot(this, '${i.id}')">${slidesHTML}</div>${dotsHTML}</div><div class="product-info"><h3>${i.name}</h3><div class="price-container">${priceHTML}</div><div class="product-actions"><button onclick="addToCart('${i.id}')" class="btn secondary-btn">Add to Cart</button><button onclick="buyNow('${i.id}')" class="btn primary-btn">Buy Now</button></div></div></div>`;
...
function handleCheckoutForm() {
    ...
    if(f) {
        f.onsubmit = async (e) => {
            ...
            const ord = { id: id, date: new Date().toLocaleDateString(), customer: { name: f.name.value, phone: f.phone.value, address: f.address.value }, items: c, total: total, status: 'Pending' };
...
// [FIXED] CONTACT FORM (Using 'message' key)
    ...
    if(f) {
        f.onsubmit = async (e) => {
            ...
            const m = {
                ...
                message: f.message.value, // FIXED KEY 'message' instead of 'text'
                isRead: false
            };
...
function initAdminProducts() {
    ...
    const render = async () => {
        ...
        tb.innerHTML = p.map((x,i)=> `<tr><td><img src="${x.images[0]||''}" style="width:40px;height:40px;object-fit:cover;border-radius:4px;"></td><td>${x.name}</td><td>৳${x.price}</td><td><button onclick="delP(${i})" style="color:red;border:none;background:none;">Del</button></td></tr>`).join('');
    ...
    if(f) {
        f.addEventListener('submit', async (e) => {
            ...
            let imgData = []; if(files.length > 0) imgData = await readFiles(files);
            const p = await getStorage(KEY_PRODUCTS);
...
function initAdminMessages() {
    ...
    const ren=async()=>{ 
        ...
        // Using 'message' property correctly
        tb.innerHTML = l.map(m => { 
            ...
            return `<tr>
                ...
                <td>${m.subject}</td>
                <td style="max-width:300px; overflow:hidden;">${m.message}</td> <td>${!m.isRead?`<button onclick="mkR(${ix})" style="color:green;background:none;border:none;">Read</button>`:''}<button onclick="delMsg(${ix})" style="color:red;background:none;border:none;">Del</button></td>
            </tr>`; 
        }).join('');
    };
    ren();
