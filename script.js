...
// Features: IndexedDB, Multi-Image, Secure Admin, Checkout
// ======================================================
...
const DB_NAME = "LynexDB_Ultimate_V1";
const DB_VERSION = 1;
let db;
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (page === 'index.html' || page === '') await loadProductsDisplay(true); // Home (New Arrivals)
...
async function loadProductsDisplay(isHome) {
    ...
    grid.innerHTML = p.length ? p.map(i => {
        ...
        <div class="product-card">
            ${badgeHTML}
            <div class="slider-container ${images.length > 1 ? '' : 'single-slide'}" id="slider-${i.id}">
                ${slides}${arrows}${dots}
            </div>
...
function handleContactForm() {
    ...
    if(f) {
        f.onsubmit = async (e) => {
            ...
            const ms = await getStorage(KEY_MESSAGES); ms.unshift(m); await setStorage(KEY_MESSAGES, ms); e.target.reset(); showPopup('Sent', 'Feedback Sent!', 'success');
        };
    }
}
...
function initAdminProducts() {
    ...
    if(f) {
        f.addEventListener('submit', async (e) => {
            ...
            // Read Files Helper
            const readFiles = (fileList) => Promise.all(fileList.map(f => new Promise(r => { 
            ...
            await setStorage(KEY_PRODUCTS, p); e.target.reset(); render(); showPopup('Success', 'Product Added!', 'success');
        });
    }
...
function initAdminOrders() {
    ...
    const ren=async()=>{ 
        ...
        document.querySelectorAll('.filter-btn').forEach(b => {
            if(b.innerText.includes(flt) || (flt==='All'&&b.innerText==='All') || (flt==='Delivered'&&b.innerText==='Completed')) b.classList.add('active'); else b.classList.remove('active');
        });

        if(l.length===0) { tb.innerHTML='<tr><td colspan="5" style="text-align:center;">No Orders Found</td></tr>'; return; }
