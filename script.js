// ======================================================
// LYNEX MAIN SCRIPT (IndexedDB + Slider + Fixed Logic)
// ======================================================
...
async function loadProductsDisplay(isHome) {
    ...
    grid.innerHTML = p.length ? p.map(i => {
        ...
        return `
        <div class="product-card">${badgeHTML}
...
async function loadCartDisplay() {
    ...
    if(cart.length===0) { c.innerHTML='<p style="text-align:center;color:#aaa;">Empty Cart</p>'; if(t) t.innerText='0'; if(document.querySelector('.checkout-btn')) document.querySelector('.checkout-btn').style.display='none'; return; }
...
function initAdminProducts() {
    ...
    const input=document.getElementById('imageInput'); const prev=document.getElementById('image-preview-container');
    tempImages = [];
    ...
    window.previewImages = () => {
        ...
        files.forEach(file => {
            ...
            r.onload = (e) => {
                tempImages.push(e.target.result); // Store base64 for saving
                prev.innerHTML += `<img src="${e.target.result}" style="width:50px;height:50px;object-fit:cover;border:1px solid #ff9f43;margin-right:5px;">`;
            };
    ...
    const render = async () => {
        ...
        tb.innerHTML = p.map((x,i)=> {
            ...
            return `<tr><td><img src="${thumb}" style="width:40px;height:40px;object-fit:cover;"></td><td>${x.name}</td><td>৳${x.price}</td><td><button onclick="delP(${i})" style="color:red;border:none;background:none;cursor:pointer;">Del</button></td></tr>`;
        }).join('');
    ...
    if(f) {
        f.addEventListener('submit', async (e) => {
            e.preventDefault();
            // Ensure tempImages populated if user didn't change anything but file is there
            if(tempImages.length === 0 && input.files.length > 0) {
               // Force read if not previewed
               const files = Array.from(input.files);
               const promises = files.map(file => new Promise((resolve) => {
                   const r = new FileReader(); r.onload = (ev) => resolve(ev.target.result); r.readAsDataURL(file);
               }));
               tempImages = await Promise.all(promises);
            }
            ...
            e.target.reset(); tempImages=[]; if(prev) prev.innerHTML='';
            render(); showPopup('Success', 'Product Added!', 'success');
        });
    }
...
// ... (Dashboard, Orders, Messages - Same as before) ...
