// ======================================================
// LYNEX MAIN SCRIPT (Feedback & Slider Dots Fixed)
// ======================================================
...
// --- DB SETUP ---
const DB_NAME = "LynexDB_Final_V6"; // Version Updated
const DB_VERSION = 1;
let db;
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (document.querySelector('.sidebar')) {
        ...
        if (document.getElementById('stat-revenue')) await initAdminDashboard();
        if (document.getElementById('add-product-form')) initAdminProducts();
        if (document.getElementById('orders-table')) initAdminOrders();
        if (document.getElementById('messages-table')) initAdminMessages(); // Feedback Fix
    }
    
    // Login
    const loginForm = document.getElementById('secure-login-form');
    if (loginForm) {
        if(sessionStorage.getItem(KEY_ADMIN_TOKEN)) window.location.href = 'x_master_v9.html';
        loginForm.addEventListener('submit', (e) => {
            e.preventDefault();
            if (e.target.username.value === "SysMaster_99" && e.target.password.value === "L7n@x#Super!2025") {
                sessionStorage.setItem(KEY_ADMIN_TOKEN, Math.random().toString(36));
                window.location.href = 'x_master_v9.html';
            } else { showPopup('Error', 'Access Denied!', 'error'); }
        });
    }
});

// --- WEBSITE DISPLAY (Dots Inside Image Fix) ---
async function loadProductsDisplay(isHome) {
    ...
    grid.innerHTML = p.length ? p.map(i => {
        ...
        let slides = images.map((src, idx) => `<img src="${src}" class="slider-image" alt="${i.name}">`).join('');
        ...
        <div class="product-card">
            ...
            <div class="image-wrapper">
                <div class="slider-container" id="slider-${i.id}" onscroll="updateActiveDot(this, '${i.id}')">
                    ${slides}
                </div>
                ${dots} </div>
...
function initAdminMessages() {
    const tb = document.querySelector('#messages-table tbody'); // Target TBODY directly
    let viewMode = 'New'; 
    ...
    const render = async () => {
        ...
        if (list.length === 0) { 
            tb.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">No Feedback Found</td></tr>'; 
        } else {
            tb.innerHTML = list.map(m => {
                ...
                <tr>
                    ...
                    <td style="text-align:right;">
                        ${!m.isRead ? `<button onclick="mkR(${idx})" style="color:#2ecc71;border:1px solid #2ecc71;background:none;padding:5px;cursor:pointer;border-radius:4px;margin-right:5px;">Mark Read</button>` : ''}
                        <button onclick="delMsg(${idx})" style="color:#e74c3c;border:1px solid #e74c3c;background:none;padding:5px;cursor:pointer;border-radius:4px;">Del</button>
                    </td>
                </tr>`;
            }).join('');
        }
    ...
    window.mkR = async (i) => { const m = await getStorage(KEY_MESSAGES); m[i].isRead = true; await setStorage(KEY_MESSAGES, m); render(); updateAdminSidebarBadges(); };
    window.delMsg = async (i) => { if(confirm('Delete?')){ const m = await getStorage(KEY_MESSAGES); m.splice(i, 1); await setStorage(KEY_MESSAGES, m); render(); updateAdminSidebarBadges(); }};
}

// --- OTHER ADMIN FUNCTIONS (Preserved) ---
// ... (Dashboard, Products, Orders logic remains same as previous working version) ...
