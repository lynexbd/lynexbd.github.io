// ======================================================
// LYNEX MAIN SCRIPT (English Popup & Unlimited Storage)
// ======================================================
...
async function updateCartCount() {
    ...
    const t = cart.reduce((sum, i) => sum + (parseInt(i.qty)||0), 0);
...
window.addToCart = async function(id) {
    ...
    if (p) { 
        ...
        await setStorage(KEY_CART, c); await updateCartCount(); alert('Product Added!'); 
    }
};
        ...
        f.onsubmit = async (e) => {
            ...
            if(await setStorage(KEY_ORDERS, orders)) {
                ...
                alert(
                    ...
                    `Items:\n${itemsList}\n\n` +
                    `Total Amount: ৳ ${total}\n\n` +
                    `* We will contact you shortly for delivery charges & confirmation.`
                );
...
function handleContactForm() {
    const f=document.getElementById('contact-form'); if(f) f.onsubmit = async (e) => { e.preventDefault(); const m={id:Date.now(), date:new Date().toLocaleDateString(), name:e.target.name.value, email:e.target.email.value, subject:e.target.subject.value, text:e.target.message.value, isRead:false}; const ms = await getStorage(KEY_MESSAGES); ms.unshift(m); await setStorage(KEY_MESSAGES, ms); e.target.reset(); alert('Message Sent Successfully!'); };
}
