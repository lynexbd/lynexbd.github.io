// ======================================================
// LYNEX MAIN SCRIPT (Final Complete Version with Notifications)
// ======================================================
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- ADMIN PAGES ---
        ...
        checkAdminAuth();
        updateAdminSidebarBadges(); // নোটিফিকেশন চেক
...
function updateAdminSidebarBadges() {
    ...
    // বর্তমান পেজ যদি অর্ডার পেজ হয়, তাহলে ব্যাজ দেখাবে না (ধরে নেওয়া হচ্ছে দেখা হয়েছে)
    const isOrderPage = window.location.pathname.includes('admin_orders.html');

    if (hasPendingOrders && orderLink && !isOrderPage) {
        if(!orderLink.querySelector('.nav-badge')) {
            orderLink.innerHTML += ' <span class="nav-badge"></span>';
        }
    }

    // 2. Check Messages (Unread)
    ...
    const isMsgPage = window.location.pathname.includes('admin_messages.html');

    if (hasUnreadMessages && msgLink && !isMsgPage) {
        if(!msgLink.querySelector('.nav-badge')) {
            msgLink.innerHTML += ' <span class="nav-badge"></span>';
        }
    }
}
...
window.buyNow = function(id) { window.addToCart(id); window.location.href = 'checkout.html'; };
...
function handleContactForm() {
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            e.target.reset(); alert('Message Sent Successfully!');
        });
    }
}
...
function initAdminProducts() {
    ...
    const renderTable = () => {
        ...
        tbody.innerHTML = products.map((p, i) => `<tr><td><img src="${p.image || ''}" style="width:40px;height:40px;object-fit:cover;background:#333;border-radius:4px;"></td><td>${p.name} ${p.isNewArrival ? '<span style="color:#2ecc71;font-size:0.8em;">(New)</span>' : ''}</td><td>৳ ${p.price}</td><td><button onclick="deleteProduct(${i})" style="color:#e74c3c;background:none;border:none;cursor:pointer;">Delete</button></td></tr>`).join('');
...
function initAdminOrders() {
    ...
    window.updateStatus = (idx, val) => { const o = getStorage(KEY_ORDERS); o[idx].status = val; setStorage(KEY_ORDERS, o); render(); };
    window.viewOrder = (id) => { const o = getStorage(KEY_ORDERS).find(x => x.id === id); if(!o) return; const its = o.items.map(i => `- ${i.name} x${i.qty} (৳${i.price})`).join('\n'); alert(`ORDER ID: ${o.id}\n\nCustomer: ${o.customer.name}\nPhone: ${o.customer.phone}\nAddress: ${o.customer.address}\n\nItems:\n${its}\n\nTOTAL: ৳${o.total}`); };
}
...
function initAdminMessages() {
    ...
    const render = () => {
        ...
        if (list.length === 0) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">No Messages</td></tr>'; } 
            ...
            tbody.innerHTML = list.map(m => {
                ...
                return `<tr><td>${m.date}</td><td>${m.name}<br><small style="color:#aaa;">${m.email}</small></td><td>${m.subject}</td><td>${m.text}</td><td>${!m.isRead ? `<button onclick="markRead(${idx})" style="color:#2ecc71;background:none;border:1px solid #2ecc71;margin-right:5px;cursor:pointer;">Mark Read</button>` : ''}<button onclick="deleteMsg(${idx})" style="color:#e74c3c;background:none;border:none;cursor:pointer;">Del</button></td></tr>`;
            }).join('');
        }
    ...
    window.markRead = (idx) => { const msgs = getStorage(KEY_MESSAGES); msgs[idx].isRead = true; setStorage(KEY_MESSAGES, msgs); render(); };
    window.deleteMsg = (idx) => { if(confirm('Delete?')) { const msgs = getStorage(KEY_MESSAGES); msgs.splice(idx, 1); setStorage(KEY_MESSAGES, msgs); render(); }};
}
