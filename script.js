// ======================================================
// LYNEX MAIN SCRIPT (Image Queue & Slider)
// ======================================================
...
// --- ADMIN: QUEUE IMAGE LOGIC ---
let tempImages = []; // To store images before saving

function initAdminProducts() {
    const f = document.getElementById('add-product-form'); 
    const tb = document.querySelector('#product-table tbody');
    
    // Setup for Image Queue
    const imgInput = document.getElementById('imageInput');
    const addImgBtn = document.getElementById('addImageBtn');
    const imgListUI = document.getElementById('image-preview-list');
    
    // Clear temp on load
    tempImages = [];
    if(imgListUI) imgListUI.innerHTML = '';
    ...
    // Helper to render queue list
    const renderQueue = () => {
        if(!imgListUI) return;
        imgListUI.innerHTML = tempImages.map((img, i) => `
            <div class="img-list-item">
                <span>${i+1}. Image ${i+1}</span>
                <span class="img-remove-btn" onclick="removeTempImg(${i})">Remove</span>
            </div>
        `).join('');
    };
    ...
    if(f) {
        f.addEventListener('submit', async (e) => {
            ...
            // If user forgot to click Add but selected a file, add it now
